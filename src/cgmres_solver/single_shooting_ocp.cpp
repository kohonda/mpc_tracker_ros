...
void SingleShootingOCP::computeOptimalityResidual(const double time, const std::vector<double>& state_vec, const double* solution_vec, std::function<double(double)>& traj_curvature,
                                                  std::function<double(double)>& traj_speed, std::function<double(double)>& drivable_width, double* optimality_residual)
{
    ...
    {
        model_.stateFunc(tau, state_mat_[i], &(solution_vec[i * dim_control_input_and_constraints_]), traj_curvature, traj_speed, drivable_width, dx_vec_);
    ...
    {
        model_.hxFunc(tau, state_mat_[i], &(solution_vec[i * dim_control_input_and_constraints_]), lambda_mat_[i + 1], traj_curvature, traj_speed, drivable_width, dx_vec_);
        for (int j = 0; j < dim_state_; ++j)
        {
    ...
    model_.huFunc(time, state_vec, solution_vec, lambda_mat_[1], traj_curvature, traj_speed, drivable_width, optimality_residual);
    tau = time;
    for (int i = 1; i < N_; ++i, tau += delta_tau)
    {
        model_.huFunc(tau, state_mat_[i], &(solution_vec[i * dim_control_input_and_constraints_]), lambda_mat_[i + 1], traj_curvature, traj_speed, drivable_width,
                      &(optimality_residual[i * dim_control_input_and_constraints_]));
    }
}

void SingleShootingOCP::predictStateFromSolution(const double current_time, const std::vector<double>& current_state, const double* solution_vec, const double prediction_length,
                                                 std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed, std::function<double(double)>& drivable_width,
