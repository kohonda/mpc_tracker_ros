...
namespace cgmres
{
    ContinuationGMRES::ContinuationGMRES(const double T_f, const double alpha, const int N, const double finite_difference_increment, const double zeta, const int kmax)
        : continuation_problem_(T_f, alpha, N, finite_difference_increment, zeta), mfgmres_(continuation_problem_.dim_solution(), kmax), solution_initializer_(finite_difference_increment, kmax), dim_control_input_(continuation_problem_.dim_control_input()), dim_constraints_(continuation_problem_.dim_constraints()), solution_vec_(linearalgebra::NewVector(continuation_problem_.dim_solution())), solution_update_vec_(linearalgebra::NewVector(continuation_problem_.dim_solution())), initial_solution_vec_(linearalgebra::NewVector(solution_initializer_.dim_solution()))
    {
    }
    ...
    bool ContinuationGMRES::controlUpdate(const double time, const std::vector<double> &state_vec, const double sampling_period, std::function<double(double)> &traj_curvature,
    ...
    void ContinuationGMRES::initializeSolution(const double initial_time, const std::vector<double> &initial_state_vec, std::function<double(double)> &traj_curvature,
                                               std::function<double(double)> &traj_speed, std::function<double(double)> &drivable_width)
    {
        solution_initializer_.computeInitialSolution(initial_time, initial_state_vec, traj_curvature, traj_speed, drivable_width, initial_solution_vec_);
        for (int i = 0; i < continuation_problem_.N(); ++i)
        {
    ...
    double ContinuationGMRES::getErrorNorm(const double time, const std::vector<double> &state_vec, std::function<double(double)> &traj_curvatur, std::function<double(double)> &traj_speed,
                                           std::function<double(double)> &drivable_width)
    {
        return continuation_problem_.computeErrorNorm(time, state_vec, solution_vec_, traj_curvatur, traj_speed, drivable_width);
    }
