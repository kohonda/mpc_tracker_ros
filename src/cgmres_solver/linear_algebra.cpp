...
void linearalgebra::DeleteVector(double* vec) { delete[] vec; }
...
double linearalgebra::InnerProduct(const int dim, const double* vec1, const double* vec2) {
  double ans = 0;
  for (int i = 0; i < dim; ++i) {
    ans += vec1[i] * vec2[i];
  }
...
double linearalgebra::InnerProduct(const int dim, const std::vector<double>& vec1, const std::vector<double>& vec2) {
  double ans = 0;
  for (int i = 0; i < dim; ++i) {
    ans += vec1[i] * vec2[i];
  }
...
double linearalgebra::SquaredNorm(const int dim, const std::vector<double>& vec) {
  double ans = 0;
  for (int i = 0; i < dim; ++i) {
    ans += vec[i] * vec[i];
  }
