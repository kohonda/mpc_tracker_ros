...
FrenetCoordinate FrenetSerretConverter::global2frenet(const MPCCourse &mpc_course, const Pose &pose_g) {
  ...
  const double current_nearest_index =
      search_nearest_point_index(previous_nearest_index_, final_search_index, mpc_course, pose_g);
  ...
  const double current_x_f =
      accumulated_x_f_to_nearest_index_ +
      delta_xf_from_nearest_pt(current_nearest_index, backward_pt_index, forward_pt_index, mpc_course, pose_g);
...
Pose FrenetSerretConverter::frenet2global(const MPCCourse &mpc_course, const FrenetCoordinate &pose_f) const {
  ...
  const int nearest_index = (std::abs(reference_x_f.at(lower_bound_index) - pose_f.x_f) <
                             std::abs(reference_x_f.at(prev_lower_bound_index) - pose_f.x_f))
                                ? lower_bound_index
                                : prev_lower_bound_index;

  // Caluculation  (x_g^N, y_g^N) -> (x_g, y_g)
  const double PI = 3.141592653589793;
  const Eigen::Vector2d delta_vec_x_f(
      (pose_f.x_f - reference_x_f.at(nearest_index)) * std::cos(mpc_course.yaw.at(nearest_index)),
      (pose_f.x_f - reference_x_f.at(nearest_index)) * std::sin(mpc_course.yaw.at(nearest_index)));
  const Eigen::Vector2d delta_vec_y_f(pose_f.y_f * std::cos(mpc_course.yaw.at(nearest_index) + PI / 2.0),
                                      pose_f.y_f * std::sin(mpc_course.yaw.at(nearest_index) + PI / 2.0));
...
int FrenetSerretConverter::search_nearest_point_index(const int &first_point_index, const int &last_point_index,
                                                      const MPCCourse &mpc_course, const Pose &pose_g) const {
  ...
  for (int i = first_point_index; i < last_point_index; i++) {
    const double dist = std::sqrt((mpc_course.x[i] - pose_g.x) * (mpc_course.x[i] - pose_g.x) +
                                  (mpc_course.y[i] - pose_g.y) * (mpc_course.y[i] - pose_g.y));
...
int FrenetSerretConverter::extract_front_point(const int &nearest_point_index, const MPCCourse &mpc_course,
                                               const Pose &pose_g) const {
  ...
  const Eigen::Vector2d vec_nearest2pre_pt(mpc_course.x[nearest_point_index - 1] - nearest_x,
                                           mpc_course.y[nearest_point_index - 1] - nearest_y);
  const Eigen::Vector2d vec_nearest2next_pt(mpc_course.x[nearest_point_index + 1] - nearest_x,
                                            mpc_course.y[nearest_point_index + 1] - nearest_y);
...
double FrenetSerretConverter::delta_xf_from_nearest_pt(const int &nearest_point_index, const int &backward_pt_index,
                                                       const int &forward_pt_index, const MPCCourse &mpc_course,
                                                       const Pose &pose_g) const {
  // Calculate the projection of the vector from the nearest point to the car to the back2front
  const Eigen::Vector2d vec_back2front(mpc_course.x[forward_pt_index] - mpc_course.x[backward_pt_index],
                                       mpc_course.y[forward_pt_index] - mpc_course.y[backward_pt_index]);
  const Eigen::Vector2d vec_nearest2ego(pose_g.x - mpc_course.x[nearest_point_index],
                                        pose_g.y - mpc_course.y[nearest_point_index]);
