...
namespace pathtrack_tools
{
    ...
    std::pair<Pose, Twist> MPCSimulator::update_ego_state(const double current_time, const Pose &current_ego_pose_global, const Twist &ego_twist, const double *control_input_vec, const double sampling_time)
    {
        const std::array<double, MPC_STATE_SPACE::DIM> current_ego_vehicle_state = ego_pose_to_state(current_ego_pose_global, ego_twist);

        auto state_func = [this](auto current_time, auto x, auto u) { return this->two_dw_with_delay(current_time, x, u); };

        std::array<double, MPC_INPUT::DIM> control_input = {control_input_vec[MPC_INPUT::ANGULAR_VEL_YAW], control_input_vec[MPC_INPUT::ACCEL]};
    ...
    std::array<std::vector<double>, MPC_STATE_SPACE::DIM> MPCSimulator::reproduct_predivted_state(const Pose &current_ego_pose_global, const Twist &current_robot_twist, const std::array<std::vector<double>, MPC_INPUT::DIM> &control_input_series, const double &sampling_time) const
    {
        std::array<double, MPC_STATE_SPACE::DIM> current_state = ego_pose_to_state(current_ego_pose_global, current_robot_twist);
        ...
        {
            const std::array<double, MPC_INPUT::DIM> input = {control_input_series[MPC_INPUT::ANGULAR_VEL_YAW][i], control_input_series[MPC_INPUT::ACCEL][i]};
    ...
    std::array<double, MPC_STATE_SPACE::DIM> MPCSimulator::ego_pose_to_state(const Pose &ego_pose, const Twist &ego_twist) const
    {
    ...
    std::pair<Pose, Twist> MPCSimulator::ego_state_to_pose(const std::array<double, MPC_STATE_SPACE::DIM> &ego_state) const
    {
        Pose ego_pose;
