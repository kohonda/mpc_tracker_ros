...
CourseManager::CourseManager(const int curvature_smoothing_num, const double max_curvature_change_rate,
                             const double speed_reduction_rate, const double deceleration_rate_for_stop)
    : curvature_smoothing_num_{curvature_smoothing_num},
      max_curvature_change_rate_{max_curvature_change_rate},
      speed_reduction_rate_{speed_reduction_rate},
      deceleration_rate_for_stop_{deceleration_rate_for_stop} {
  nearest_index_ = 0;
  nearest_ratio_ = 1.0;
  second_nearest_index_ = 0;
  second_nearest_ratio_ = 0.0;
...
int CourseManager::get_path_size() const { return mpc_course_.size(); }

void CourseManager::clear_path() { mpc_course_.clear(); }

MPCCourse CourseManager::get_mpc_course() const { return mpc_course_; }
...
void CourseManager::set_course_from_csv(const std::string &csv_path) {
  ...
  try {
    ...
    } else if (reference_x.size() != reference_y.size() || reference_x.size() != reference_speed.size() ||
               reference_x.size() != drivable_delta_y_f.size()) {
...
std::vector<double> CourseManager::calc_accumulated_path_length(const double &offset,
                                                                const MPCCourse &mpc_course) const {
  ...
  for (size_t i = 0; i < accumulated_path_length.size(); i++) {
    ...
    if (i == 0) {
      ...
      const double delta_length =
          std::hypot(mpc_course.x[i] - mpc_course.x[i - 1], mpc_course.y[i] - mpc_course.y[i - 1]);
...
std::vector<double> CourseManager::calc_path_curvature(const int &smoothing_num, const MPCCourse &mpc_course) const {
  ...
  for (int i = L; i < path_size - L; i++) {
    ...
    const double curvature =
        2.0 * ((p2[0] - p1[0]) * (p3[1] - p1[1]) - (p2[1] - p1[1]) * (p3[0] - p1[0])) / denominator;
...
void CourseManager::deceleration_for_stop(const std::vector<double> &accumulated_path_length,
  ...
  for (size_t i = 0; i < ref_speed->size(); i++) {
    ...
    ref_speed->at(i) =
        std::max(0.0, ref_speed->at(i) *
                          (1 - std::exp(-deceleration_rate_for_stop_ * (final_x_f - accumulated_path_length[i]))));
  }
}

// Saturate the rate of curvature change when curvature_change_rate > max_curvature_change_rate
void CourseManager::filtering_based_curvature_rate(const std::vector<double> &accumulated_path_length,
  ...
  for (size_t i = 1; i < path_curvature->size(); i++) {
    ...
    ref_speed->at(i) =
        ref_speed->at(i) * std::exp(-speed_reduction_rate_ * curvature_change_rate * curvature_change_rate);
  }
  if (is_filtered) {
...
double CourseManager::get_curvature(const double &pose_x_f) {
  ...
  const double curvature_interpolated = linear_interpolate(mpc_course_.curvature, nearest_index_, nearest_ratio_,
                                                           second_nearest_index_, second_nearest_ratio_);
...
double CourseManager::get_speed(const double &pose_x_f) {
  ...
  const double reference_speed_interpolated = linear_interpolate(mpc_course_.speed, nearest_index_, nearest_ratio_,
                                                                 second_nearest_index_, second_nearest_ratio_);
...
double CourseManager::get_drivable_width(const double &pose_x_f) {
  ...
  const double drivable_width_interpolated = linear_interpolate(
      mpc_course_.drivable_width, nearest_index_, nearest_ratio_, second_nearest_index_, second_nearest_ratio_);
...
int CourseManager::search_nearest_index(const MPCCourse &mpc_course, const double &pose_x_f,
                                        const int &last_nearest_index) const noexcept {
...
int CourseManager::lookup_xf_to_nearest_index(const std::unordered_map<double, int> &hash_xf2index,
                                              const double &pose_x_f) const {
...
void CourseManager::set_internal_ratio(const MPCCourse &mpc_course, const double &pose_x_f, const int &nearest_index) {
  ...
  // Calculate internal ratio of pose_x_f between nearest and second_nearest
  const double dist_nearest_and_second = std::abs(mpc_course_.accumulated_path_length[second_nearest_index_] -
                                                  mpc_course_.accumulated_path_length[nearest_index_]);
  ...
  if (dist_nearest_and_second < 1.0e-5) {
    ...
    const double dist_to_second_nearest =
        std::abs(mpc_course_.accumulated_path_length[second_nearest_index_] - pose_x_f);
...
double CourseManager::linear_interpolate(const std::vector<double> &reference_vec, const int &nearest_index,
                                         const double &nearest_ratio, const int &second_nearest_index,
