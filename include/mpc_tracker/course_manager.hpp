...
class CourseManager {
 public:
   ...
   */
  CourseManager(/* args */);
  CourseManager(const int curvature_smoothing_num, const double max_curvature_change_rate,
 ...
 private:
  ...
  const double speed_reduction_rate_ =
      0.1;  //!< @brief Reduce the speed reference based on the rate of curvature change; v_ref' = v_ref * exp
            //!< (-speed_reduction_rate * curvature_rate^2)
  const double deceleration_rate_for_stop_ =
      0.3;  //!< @brief Reduce the speed reference for stopping; v_ref'  = v_ref * (1 - exp(-deceleration_rate_for_stop
            //!< * (x_goal -x_f))), recommend the same value as a a_min in MPC formulation
  ...
  const double redundant_xf_ =
      5.0;  // Reserve the lookup table for the extra -redundant_xf[m] from the initial value of mpc_course.
  ...
  void filtering_based_curvature_rate(const std::vector<double> &accumulated_path_length,
                                      std::vector<double> *path_curvature, std::vector<double> *ref_speed);
  ...
  inline int search_nearest_index(const MPCCourse &mpc_course, const double &pose_x_f,
  ...
  inline int lookup_xf_to_nearest_index(const std::unordered_map<double, int> &hash_xf2index,
  ...
  double linear_interpolate(const std::vector<double> &reference_vec, const int &nearest_index,
