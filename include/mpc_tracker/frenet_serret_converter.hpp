...
/// @brief Convert a position described in global coordinates to the
/// Frenet-Serret coordinate system defined by the reference path.
///
...
class FrenetSerretConverter {
 public:
   ...
   * @param current_pose_g (x_g, y_g, z_g ,roll_g, pitch_g ,yaw_g[rad]): ego
   * vehicle position based on Global coordinate
   * @return current_pose_f (x_f, y_f, yaw_f[rad]): ego vehicle position based
   * on Frenet-Serret coordinate
   */
 ...
 private:
   ...
   */
  int search_nearest_point_index(const int &first_point_index, const int &last_point_index, const MPCCourse &mpc_course,
  ...
  /**
   * @brief Calculate the projection on the reference path from the nearest
   * waypoint to the vehicle position.
   *
   ...
   */
  double delta_xf_from_nearest_pt(const int &nearest_point_index, const int &backward_pt_index,
                                  const int &forward_pt_index, const MPCCourse &mpc_course, const Pose &pose_g) const;
};
