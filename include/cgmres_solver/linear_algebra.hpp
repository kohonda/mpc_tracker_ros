...
double InnerProduct(const int dim, const std::vector<double>& vec1, const std::vector<double>& vec2);

// Returns squared norm of vec.
double SquaredNorm(const int dim, const double* vec);
double SquaredNorm(const int dim, const std::vector<double>& vec);
