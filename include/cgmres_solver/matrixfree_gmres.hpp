...
namespace cgmres
{
    ...
    public:
        ...
        // and all matrices.
        // MatrixFreeGMRES();
        MatrixFreeGMRES() : dim_linear_problem_(0), kmax_(0), hessenberg_mat_(nullptr), basis_mat_(nullptr), b_vec_(nullptr), givens_c_vec_(nullptr), givens_s_vec_(nullptr), g_vec_(nullptr)
        {
        }
        ...
        MatrixFreeGMRES(const int dim_linear_problem, const int kmax)
            : dim_linear_problem_(dim_linear_problem), kmax_(kmax), hessenberg_mat_(linearalgebra::NewMatrix(kmax + 1, kmax + 1)), basis_mat_(linearalgebra::NewMatrix(kmax + 1, dim_linear_problem)), b_vec_(linearalgebra::NewVector(dim_linear_problem)), givens_c_vec_(linearalgebra::NewVector(kmax + 1)), givens_s_vec_(linearalgebra::NewVector(kmax + 1)), g_vec_(linearalgebra::NewVector(kmax + 1))
        {
        ...
        bool solveLinearProblem(LinearProblemGenerator &linear_problem_generator, LinearProblemArgs... linear_problem_args, double *solution_vec)
        {
            bool is_mpc_solved = true; // for catch error in cgmres
            ...
            linear_problem_generator.bFunc(linear_problem_args..., solution_vec, b_vec_);
            g_vec_[0] = std::sqrt(linearalgebra::SquaredNorm(dim_linear_problem_, b_vec_));
            ...
            for (k = 0; k < kmax_; ++k)
            {
                linear_problem_generator.AxFunc(linear_problem_args..., basis_mat_[k], basis_mat_[k + 1]);
                for (int j = 0; j <= k; ++j)
                {
                    hessenberg_mat_[k][j] = linearalgebra::InnerProduct(dim_linear_problem_, basis_mat_[k + 1], basis_mat_[j]);
                ...
                hessenberg_mat_[k][k + 1] = std::sqrt(linearalgebra::SquaredNorm(dim_linear_problem_, basis_mat_[k + 1]));
    ...
    private:
        ...
        inline void givensRotation(double *column_vec, const int i_column)
        {
            double tmp1 = givens_c_vec_[i_column] * column_vec[i_column] - givens_s_vec_[i_column] * column_vec[i_column + 1];
            double tmp2 = givens_s_vec_[i_column] * column_vec[i_column] + givens_c_vec_[i_column] * column_vec[i_column + 1];
