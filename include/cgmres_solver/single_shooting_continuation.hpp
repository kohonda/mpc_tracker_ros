...
class SingleShootingContinuation
{
    ...
    SingleShootingContinuation(const double T_f, const double alpha, const int N, const double finite_difference_increment, const double zeta);
    ...
    //  zeta: A parameter for stabilization of the C/GMRES method. It may work
    //    well to set this parameters as the reciprocal of the sampling period.
    SingleShootingContinuation(const double T_f, const double alpha, const int N, const double initial_time, const double finite_difference_increment, const double zeta);
    ...
    void integrateSolution(double* solution_vec, const double* solution_update_vec, const double integration_length);

    // Computes and returns the squared norm of the errors in optimality under
    // the state_vec and the current solution.
    double computeErrorNorm(const double time, const std::vector<double>& state_vec, const double* solution_vec, std::function<double(double)>& traj_curvature,
    ...
    double *incremented_solution_vec_, *optimality_residual_, *optimality_residual_1_, *optimality_residual_2_;
    // std::vector<double> optimality_residual_, optimality_residual_1_, optimality_residual_2_;
    std::vector<double> incremented_state_vec_;
};
