...
#include "cgmres_solver/matrixfree_gmres.hpp"
...
class ContinuationGMRES
{
    ...
    //     GMRES method.
    ContinuationGMRES(const double T_f, const double alpha, const int N, const double finite_difference_increment, const double zeta, const int kmax);
    ...
    bool controlUpdate(const double time, const std::vector<double>& state_vec, const double sampling_period, std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed,
                       std::function<double(double)>& drivable_width, double* control_input_vec, std::array<std::vector<double>, MPC_INPUT::DIM>* control_input_series);
    ...
    void setParametersForInitialization(const double* initial_guess_solution, const double newton_residual_tolerance, const int max_newton_iteration);
    ...
    void initializeSolution(const double initial_time, const std::vector<double>& initial_state_vec, std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed,
    ...
    double getErrorNorm(const double time, const std::vector<double>& state_vec, std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed,
                        std::function<double(double)>& drivable_width);
    ...
    private:
    // SingleShootingContinuation continuation_problem_;
    MatrixFreeGMRES<SingleShootingContinuation, const double, const std::vector<double>&, const double*, std::function<double(double)>&, std::function<double(double)>&, std::function<double(double)>&>
        mfgmres_;
