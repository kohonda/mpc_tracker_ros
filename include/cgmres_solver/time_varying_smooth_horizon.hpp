...
class TimeVaryingSmoothHorizon {
 public:
  ...
  TimeVaryingSmoothHorizon(const double T_f, const double alpha, const double initial_time);
  ...
  inline double getLength(const double time) const { return T_f_ * (1.0 - std::exp(-alpha_ * (time - initial_time_))); }

  // Resets the parameters of the horizon.
  void resetLength(const double initial_time);

  // Resets the parameters of the horizon.
  void resetLength(const double T_f, const double alpha, const double initial_time);
