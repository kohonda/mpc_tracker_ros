...
#include "cgmres_solver/newton_gmres_for_ocp.hpp"
...
class CGMRESInitializer
{
    ...
    CGMRESInitializer(const double finite_difference_increment, const int kmax, const double newton_residual_tolerance, const int max_newton_iteration);
    ...
    //     iteration for the initialization terminates when the number of the
    //     iteration is equal to this value.
    void setCriterionsOfNewtonTermination(const double newton_residual_tolerance, const int max_newton_iteration);
    ...
    void computeInitialSolution(const double initial_time, const std::vector<double>& initial_state_vec, std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed,
                                std::function<double(double)>& drivable_width, double* initial_solution_vec);
    ...
    void getInitialLambda(const double initial_time, const std::vector<double>& initial_state_vec, std::function<double(double)>& traj_curvature, std::function<double(double)>& traj_speed,
                          std::function<double(double)>& drivable_width, std::vector<double>& initial_lambda_vec);
    ...
    MatrixFreeGMRES<NewtonGMRESForOCP<ZeroHorizonOCP>, const double, const std::vector<double>&, const double*, std::function<double(double)>&, std::function<double(double)>&,
